...
const AllModels = () => {
  ...
  const handleSort = (frame) => {
    if (frame === "All") {
      setModels(forFramework);
    } else {
      const filteredModel = forFramework.filter(
        (model) => model.framework === frame
      );
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-12 py-10">
      ...
      {models.length === 0 ? (
        <h1 className="text-secondary italic font-semibold text-3xl text-center">
          No Models Found
        </h1>
      ) : (
